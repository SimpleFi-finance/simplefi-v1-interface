...
async function depositETHToDAIAave() {
    console.log("-------------------------------------------------------------------------------------");
    console.log("Deposit ETH to Aave wBTC pool");
    const awBTCContract = await ethers.getContractAt("IERC20", awBTC);
    console.log("Initial balance of awBTC: ", (await awBTCContract.balanceOf(account.address)).toString());
    ...
    console.log("Final balance of awBTC: ", (await awBTCContract.balanceOf(account.address)).toString());
}

async function migrateDAIFromAaveToCurve() {
    console.log("-------------------------------------------------------------------------------------");
    console.log("Migrate BTC from Aave to Curve");
    const awBTCContract = await ethers.getContractAt("IERC20", awBTC);
    const curveLPTokenContract = await ethers.getContractAt("IERC20", CURVE_AAVE_LP_TOKEN);

    const amount = (await awBTCContract.balanceOf(account.address)).toString();

    console.log("Initial balance of awBTC: ", amount);
    console.log("Initial balance of curve LPToken: ", (await curveLPTokenContract.balanceOf(account.address)).toString());

    const adapter = await controller.getAdapterAddressForMarket(awBTC);
    await approve(awBTC, adapter, amount);
